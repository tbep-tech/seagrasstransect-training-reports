...
It should be abundantly clear that using the weighted mean of the absolute differences is a more robust approach than simply averaging the raw deviations across species.  The weighted mean accounts for the fact that some species are more variable than others, and therefore should contribute less to the overall score.  This is particularly important when there are a small number of species, as is often the case in seagrass transects.

As an example, if there was no deviation in scores among species, $\sigma_s$ would be 0 for all species, and the weights $w_s$ would all be 1.  The numerator $N$ would then be the sum of the absolute deviations, and the denominator $D$ would be the sum of the weights.  The raw metric score would then be the average of the absolute deviations.
...
The raw Abundance score would then be $N/D = 2/3 = 0.667$, which is lower than the weighted mean.
